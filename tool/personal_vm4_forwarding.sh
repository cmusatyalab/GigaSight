#!/bin/bash
GUEST_IP=`sudo ./virt-addr.py private-vm4`
echo "port forwarding for ${GUEST_IP}"

sudo ./portforwarding.sh personal_vm stopped 2232 ${GUEST_IP} 22 >/dev/null 2>&1
sudo ./portforwarding.sh personal_vm start 2232 ${GUEST_IP} 22
