...
public class RandomFileUploader implements FileUploader {
	...
	private final static String TAG = "RandomFileUploader";
	private int chunkSec;
	private File videoDir;
	private Timer timer;
	private boolean loopUpload;
	private boolean startRandomChunk;
	/* Upload files without waiting for a signal from the server */
	public RandomFileUploader(ControllerClientv2 controller, String uploadIP, int uploadPort, int chunkSec, File videoDir, boolean loopUpload,boolean startRandom) {
	...
	public void run() {
		...
		controller.sendMessage(MainActivity.UPDATE_STATUS,"RandomFileUploader");
		//start filling the queue with files to be uploaded
		UploadTask upTask = new UploadTask(this.startRandomChunk);
		int startInterval = new Random(System.currentTimeMillis()).nextInt(chunkSec*1000-500);
		controller.sendMessage(MainActivity.UPDATE_STATUS, "Random start: "+startInterval);
		timer.scheduleAtFixedRate(upTask, startInterval, chunkSec*1000); 
		...
		while(keepRunning){
			try {
				controller.sendMessage(MainActivity.UPDATE_STATUS,"Waiting for upload to become ready");
				Log.d(TAG,"Waiting for file to become ready");
	...
	private class UploadTask extends TimerTask{
		...
		public void run() {
			try {				
				...
				counter++;
				if(RandomFileUploader.this.loopUpload){
