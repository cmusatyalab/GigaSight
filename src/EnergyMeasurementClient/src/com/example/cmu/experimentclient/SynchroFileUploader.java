...
public class SynchroFileUploader implements FileUploader {
	...
	public SynchroFileUploader(ControllerClientv2 controller, String uploadIP, int uploadPort, int chunkSec, File videoDir, boolean loopUpload,boolean startRandom) {
		...
		this.startRandomChunk = startRandom;
	}

	public void run() {
		controller.sendMessage(MainActivity.UPDATE_STATUS,"Synchro file uploader");
		// connect to uploadIP
		try {
		...
		controller.sendMessage(MainActivity.UPDATE_STATUS,"SynchroFileUploader");
		//start filling the queue with files to be uploaded
		UploadTask upTask = new UploadTask(this.startRandomChunk);
		timer.scheduleAtFixedRate(upTask, 1000, chunkSec*1000); //wait one second before writing first file
		...
		while(keepRunning){
			try {
				controller.sendMessage(MainActivity.UPDATE_STATUS,"Waiting for send signal");
				signal.clear();
				bReadBlocked = true;
				sockChannel.read(signal); //this will block until a signal is received
				Log.i(TAG,"position: "+signal.position()+signal.limit());
				bReadBlocked = false;
				if(signal.hasRemaining()){
					Log.i(TAG,"zero read...");
					controller.sendMessage(MainActivity.UPDATE_STATUS,"ZERO READ");
					continue; //zero read?
				}
				controller.sendMessage(MainActivity.UPDATE_STATUS,"Send signal received, waiting for upload to become ready");
				Log.d(TAG,"Uploading signal received, waiting for file to become ready");
	...
	private class UploadTask extends TimerTask{
		...
		public void run() {
			try {				
				...
				counter++;
				if(SynchroFileUploader.this.loopUpload){
