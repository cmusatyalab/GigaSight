...
CLOUDLET_RESOURCE = "http://monsoon.elijah.cs.cmu.edu:5000"
...
def calculate_class(arg):
    ...
    frame_counter = arg[1]
    name = "indexing"
    image = Image.open("tmp/" + name + "%03d.png" % (frame_counter + 1))
...
''' 
            seg_id = stream.get("segment").rsplit('/')[-2]
            seg_id_1 = seg_id[-1]
            seg_id_2 = seg_id[-2]
            if not seg_id_2.isdigit():
                seg_id_2 = '0'
            seg_id = int(seg_id_2 + seg_id_1)
            if seg_id % 4 == 1 or seg_id % 4 == 2:
                continue
'''

            new_flag = True
            frame_rate = 1
            log_file = open("log_throughput.txt", "a")
            ...
            if os.path.isdir("tmp"):
                os.system("rm -rf tmp/")
            os.makedirs("tmp")
            os.system("ffmpeg -i " + os.path.join(path, name) + " -vsync 2 -f image2 tmp/indexing%03d.png")
            ...
            obj_list = [os.path.split(x)[1] for x in glob.glob(os.path.join('tmp', '*.png'))]
            key_frame_num = len(obj_list)
            pool.map(calculate_class, [(q, i) for i in xrange(key_frame_num)])
            ...
            while not q.empty():
                ...
                for class_index, score in enumerate(ILP): 
                    if score > SCORE_THRESHOLD:
                        ILP_list[class_index].append((key_frame_counter, score))
            ...
            for class_index, frame_list in enumerate(ILP_list):
                if not frame_list:
                    continue
                frame_list_split = split_frame_list(frame_list, 2)
            ...
            print "total time: %.2f, key frames: %d, frame per sec: %.2f" \
                   % (elapse_time, key_frame_num, key_frame_num / elapse_time)
            print
            log_file.write("start: %.2f, end: %.2f, elapse: %.2f, fps: %.2f\n" % (start_time, start_time + elapse_time, elapse_time, key_frame_num / elapse_time))
            log_file.close()
